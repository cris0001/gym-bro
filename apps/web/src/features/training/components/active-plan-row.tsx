...
export function ActivePlanRow({ planId, name }: ActivePlanRowProps) {
  ...
  return (
    <div className="bg-card overflow-hidden rounded-2xl border">
      ...
      {expanded &&
        (templates.length === 0 ? (
          <p className="text-muted-foreground border-t border-dashed border-[#e4dad2] px-4 py-3 text-sm dark:border-[#2f292d]">
            This plan has no templates yet.
          </p>
        ) : (
          <ul className="divide-y divide-dashed divide-[#e4dad2] border-t border-dashed border-[#e4dad2] dark:divide-[#2f292d] dark:border-[#2f292d]">
            {templates.map((template, i) => {
              ...
              return (
                ...
                >
                  <GripVertical className="size-4 shrink-0 text-[#c9bcb2] dark:text-[#5c5358]" />
                  <Link
                  ...
                  <ChevronRight className="size-4 shrink-0 text-[#c9bcb2] dark:text-[#5c5358]" />
                </li>
              );
            })}
          </ul>
