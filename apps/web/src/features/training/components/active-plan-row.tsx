...
export function ActivePlanRow({ planId, name }: ActivePlanRowProps) {
  ...
  return (
    <div className="bg-card overflow-hidden rounded-2xl border">
      ...
      {expanded &&
        (templates.length === 0 ? (
          <p className="text-muted-foreground border-t border-dashed border-[#e5d9c6] px-4 py-3 text-sm">
          ...
          <ul className="divide-y divide-dashed divide-[#e5d9c6] border-t border-dashed border-[#e5d9c6]">
            {templates.map((template, i) => {
              ...
              return (
                ...
                >
                  <GripVertical className="size-4 shrink-0 text-[#c9bda9]" />
                  <Link
                  ...
                  <ChevronRight className="size-4 shrink-0 text-[#c9bda9]" />
                </li>
              );
            })}
          </ul>
